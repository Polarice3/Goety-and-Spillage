...
public class GSWebNet extends PathfinderMob implements IllagerAttack {
    private static final EntityDataAccessor<Float> ATTACH_POINT_X = SynchedEntityData.defineId(GSWebNet.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> ATTACH_POINT_Y = SynchedEntityData.defineId(GSWebNet.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> ATTACH_POINT_Z = SynchedEntityData.defineId(GSWebNet.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Boolean> CAUGHT = SynchedEntityData.defineId(GSWebNet.class, EntityDataSerializers.BOOLEAN);
    ...
    public GSWebNet(EntityType<? extends PathfinderMob> p_21683_, Level p_21684_) {
    ...
    public void tick() {
        ...
        for (Entity entity : list){
            if (!this.shouldReturn && !this.isCaught() && entity instanceof LivingEntity living) {
                boolean canHurt = attacker instanceof Mob mob ? !MobUtil.areAllies(living, mob) : living != this.shooter;
                if (canHurt && entity.isAlive() && !entity.isInvulnerable() && !entity.isSpectator()) {
                    if (!this.level.isClientSide) {
                        this.setCaught(true);
                    }

                    living.hurt(this.damageSources().thrown(this, (Entity)attacker), 2.0F);
        ...
        if (!this.level.isClientSide && this.shooter != null && this.shooter.isAlive()) {
            float radius2 = -2.0F;
            double x2 = this.shooter.getX() + 0.8D * Math.sin((double)(-this.shooter.getYRot()) * Math.PI / 180.0) + (double)radius2 * Math.sin((double)(-this.shooter.yHeadRot) * Math.PI / 180.0) * Math.cos((double)(-this.shooter.getXRot()) * Math.PI / 180.0);
            double z2 = this.shooter.getZ() + 0.8D * Math.cos((double)(-this.shooter.getYRot()) * Math.PI / 180.0) + (double)radius2 * Math.cos((double)(-this.shooter.yHeadRot) * Math.PI / 180.0) * Math.cos((double)(-this.shooter.getXRot()) * Math.PI / 180.0);
            this.setAttachPoint(x2, this.shooter.getY() + 1.0, z2);
        }

        if (!this.isCaught()) {
            this.setDeltaMovement(this.accelerationX, this.accelerationY, this.accelerationZ);
        } else if (this.pullingEntity != null && this.pullingEntity.isAlive()) {
            this.setPos(this.pullingEntity.getBoundingBox().getCenter());
        } else if (!this.level.isClientSide) {
            this.setCaught(false);
        }

        if (this.pullingEntity != null && this.distanceToSqr(this.pullingEntity) <= 4.0) {
        ...
        if (this.shouldReturn && this.shooter != null) {
            ...
            if (this.distanceToSqr(this.shooter) < 12.25) {
                if (this.pullingEntity != null) {
                    this.pullingEntity.setDeltaMovement(0.0, 0.0, 0.0);
                    if (this.pullingEntity instanceof ServerPlayer serverPlayer) {
                        serverPlayer.connection.send(new ClientboundSetEntityMotionPacket(this.pullingEntity.getId(), new Vec3(0.0, 0.0, 0.0)));
                    }
                }

                if (this.shooter instanceof RagnoServant ragno) {
    ...
    public Vec3 getAttachPoint() {
        return new Vec3((double) this.entityData.get(ATTACH_POINT_X), (double)(Float)this.entityData.get(ATTACH_POINT_Y), (double)(Float)this.entityData.get(ATTACH_POINT_Z));
    }
