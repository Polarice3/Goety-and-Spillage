...
public class BoundEngineer extends AbstractBoundIllager implements ICanBeAnimated {
    private static final EntityDataAccessor<Integer> ANIMATION_STATE = SynchedEntityData.defineId(BoundEngineer.class, EntityDataSerializers.INT);
    ...
    public BoundEngineer(EntityType<? extends Owned> type, Level worldIn) {
        super(type, worldIn);
    }
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Monster.createMonsterAttributes()
                ...
                .add(Attributes.MAX_HEALTH, GSAttributesConfig.BoundEngineerHealth.get())
                .add(Attributes.ARMOR, GSAttributesConfig.BoundEngineerArmor.get())
                .add(Attributes.ATTACK_DAMAGE, GSAttributesConfig.BoundEngineerDamage.get())
                .add(Attributes.FOLLOW_RANGE, 64.0D);
    }

    @Override
    public void setConfigurableAttributes() {
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), GSAttributesConfig.BoundEngineerHealth.get());
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), GSAttributesConfig.BoundEngineerDamage.get());
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), GSAttributesConfig.BoundEngineerArmor.get());
    }
    ...
    class EngineerAvoidEntityGoal<T extends LivingEntity> extends AvoidTargetGoal<T> {
        ...
        public boolean canUse() {
            return super.canUse() && BoundEngineer.this.attackType == 0;
        }

        public boolean canContinueToUse() {
            return super.canContinueToUse() && BoundEngineer.this.attackType == 0;
        }
    }
    ...
    class ThrowMachineGoal extends Goal {
        ...
        public void start() {
            ...
            BoundEngineer.this.attackType = THROW_ATTACK;
        }
        ...
        public void stop() {
            ...
            BoundEngineer.this.throwCooldown = BoundEngineer.this.random.nextInt(300, 501);
        }
    }
    ...
    class RepairGoal extends Goal {
        ...
        public boolean canUse() {
            boolean initialConditions = BoundEngineer.this.attackType == 0 && BoundEngineer.this.repairCooldown < 1;
            if (BoundEngineer.this.isStaying() || BoundEngineer.this.isCommanded()){
                ...
                BoundEngineer.this.toRepair = closestToRepair;
                this.path = BoundEngineer.this.toRepair == null ? null : BoundEngineer.this.getNavigation().createPath(BoundEngineer.this.toRepair, 0);
                return this.path != null;
            }
        }

        public void start() {
            BoundEngineer.this.attackType = REPAIR_ATTACK;
            BoundEngineer.this.getNavigation().stop();
            BoundEngineer.this.getNavigation().moveTo(this.path, 1.0);
        }

        public boolean canContinueToUse() {
            if (BoundEngineer.this.isStaying() || BoundEngineer.this.isCommanded()){
                return false;
            } else if ((BoundEngineer.this.repairTicks <= 0 || !(BoundEngineer.this.distanceToSqr(BoundEngineer.this.toRepair) > 4.0)) && BoundEngineer.this.toRepair.isAlive()) {
                return BoundEngineer.this.toRepair != null && BoundEngineer.this.isAlive() && BoundEngineer.this.toRepair.getHealth() < BoundEngineer.this.toRepair.getMaxHealth() && BoundEngineer.this.repairTicks > 0 ? BoundEngineer.this.repairTicks <= 40 : BoundEngineer.this.attackTicks <= 300;
            } else {
                BoundEngineer.this.getNavigation().stop();
                return false;
            }
        ...
        public void tick() {
            ...
            BoundEngineer.this.setTarget(null);
            if (BoundEngineer.this.toRepair != null) {
                BoundEngineer.this.getLookControl().setLookAt(BoundEngineer.this.toRepair, 100.0F, 100.0F);
            }
    ...
    class EngineerRandomStrollGoal<T extends PathfinderMob & IServant> extends WanderGoal<T> {
        ...
        public void stop() {
            if (BoundEngineer.this.attackType != REPAIR_ATTACK) {
                super.stop();
            }
