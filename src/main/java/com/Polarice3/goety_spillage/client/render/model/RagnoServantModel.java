...
public class RagnoServantModel<T extends Entity> extends HierarchicalModel<T> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(GoetySpillage.location("ragno"), "main");
    ...
    public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        float craziness = 1.0F;
        if (entity instanceof RagnoServant ragno) {
            craziness = ragno.isCrazy() ? 2.0F : 1.0F;
            if (ragno.deathTime > 40){
                craziness = 0.0F;
            }
        }

        head.yRot = netHeadYaw * 0.017453292F + (-0.5F + this.random.nextFloat()) * 0.08F * craziness;
        head.xRot = -0.0873F + headPitch * 0.017453292F + (-0.5F + this.random.nextFloat()) * 0.08F * craziness;
        head.zRot = (-0.5F + this.random.nextFloat()) * 0.08F * craziness;
        neck.yRot = (-0.5F + this.random.nextFloat()) * 0.04F * craziness;
        neck.xRot = (-0.5F + this.random.nextFloat()) * 0.04F * craziness;
        abdomen.yRot = (-0.5F + this.random.nextFloat()) * 0.16F * craziness;
        abdomen.xRot = -0.3491F + (-0.5F + this.random.nextFloat()) * 0.08F * craziness;
        abdomen.zRot = (-0.5F + this.random.nextFloat()) * 0.08F * craziness;
        ...
        leg1.zRot += -$$16;
        if (entity instanceof RagnoServant ragno) {
            this.animate(ragno.introAnimationState, RagnoAnimation.INTRO, ageInTicks, ragno.getAnimationSpeed());
            this.animate(ragno.phaseAnimationState, RagnoAnimation.PHASE, ageInTicks, ragno.getAnimationSpeed());
            this.animate(ragno.blockAnimationState, RagnoAnimation.BLOCK, ageInTicks, ragno.getAnimationSpeed());
            this.animate(ragno.webAnimationState, RagnoAnimation.WEB, ageInTicks, ragno.getAnimationSpeed());
            this.animate(ragno.leapAnimationState, RagnoAnimation.LEAP, ageInTicks, ragno.getAnimationSpeed());
            this.animate(ragno.burrowAnimationState, RagnoAnimation.BURROW, ageInTicks, ragno.getAnimationSpeed());
            this.animate(ragno.popupAnimationState, RagnoAnimation.POPUP, ageInTicks, ragno.getAnimationSpeed());
            this.animate(ragno.chargeAnimationState, RagnoAnimation.CHARGE, ageInTicks, ragno.getAnimationSpeed());
            this.animate(ragno.coughAnimationState, RagnoAnimation.COUGH, ageInTicks, ragno.getAnimationSpeed());
            this.animate(ragno.stunAnimationState, RagnoAnimation.STUN, ageInTicks, ragno.getAnimationSpeed());
        }
